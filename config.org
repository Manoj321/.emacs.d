...
#+BEGIN_SRC emacs-lisp
  (use-package org
    ...
    :config (setq org-directory '("~/Dropbox/org")
                  ...
                  org-columns-default-format "%70ITEM(Task) %2PRIORITY %10Effort(Effort){:} %10CLOCKSUM"
...
** Highlight clocked in status
   copied [[https://github.com/iqbalansari/dotEmacs/blob/master/config/workflow.org#better-display-of-clocked-in-task-also-indicate-if-not-currently-clocked-in][from]]
   #+begin_src emacs-lisp
     ...
     (defun iqbal-org-clock-mode-line ()
       (let ((clocking-p (and (fboundp 'org-clocking-p)
         ...
         (let ((mode-line-string (format " %s "
           ...
           (format "%s " (propertize mode-line-string 'face mode-line-face)))))
